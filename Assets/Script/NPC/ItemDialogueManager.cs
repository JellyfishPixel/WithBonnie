...
public class ItemDialogueManager : MonoBehaviour
{
    ...
    Vector2 cachedCameraLook;
    bool hasCachedLook;
    float cachedPitch;
    Vector2 cachedLook;
    bool hasCachedCamera;
    ...
    IEnumerator ReleaseDialogueAndResetJump()
    {
        ...
        if (camMgr != null)
        {
            if (camMgr.CurrentMode == CameraMode.FirstPerson)
            {
                camMgr.ResetActiveControllerOneFrame();
            }
            else
            {
                ResetThirdPersonControllerSafely();
            }
        }
    ...
    void ResetThirdPersonControllerSafely()
    {
        ...
        var tps = camMgr.thirdPersonController;
        if (!tps) return;

        var cc = tps.GetComponent<CharacterController>();
        if (!cc) return;

        // ปิดชั่วคราว 1 frame
        tps.enabled = false;

        // รีเซ็ตศูนย์กลาง CharacterController
        ...
        Debug.Log("[Dialogue] Reset ThirdPerson Controller after dialogue");
    }
