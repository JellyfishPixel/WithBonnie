...
public class GameManager : MonoBehaviour
{
    ...
    public void DevRegisterTestDelivery(string testDestinationId)
    {
        // 🔧 เปลี่ยน ID ตรงนี้ได้ตามที่อยากเทส
        testDestinationId = "Home1";
    ...
    void Update()
    {
        ...
        {
            DevRegisterTestDeliveryById("Home1");
        } 
        if (Input.GetKeyDown(KeyCode.F2))
        {
            DevRegisterTestDeliveryById("Home2");
        }
        UpdateGameTime();
    }
    ...
    {
        directionArrowUI = FindFirstObjectByType<DirectionArrowUI>();
        minimap = FindFirstObjectByType<MinimapController>();
    ...
    public void RelinkSceneSystemsAndRebuildMinimap()
    {
        ...
        foreach (var rec in activeBoxes)
        {
            ...
            if (rec.worldTarget == null && directionArrowUI != null)
            {
                rec.worldTarget = directionArrowUI.warpTarget;
            }
    ...
    public void RegisterNewDelivery(PackedBoxData package)
    {
        ...
        if (record.worldTarget == null && directionArrowUI != null)
        {
            Debug.Log("[GM] Destination not in this scene → using warpTarget");
            record.worldTarget = directionArrowUI.warpTarget;
        }
        ...
        {
            bool stillHasSameTarget = false;

            foreach (var r in activeBoxes)
            {
                if (r != record && r.worldTarget == record.worldTarget)
                {
                    stillHasSameTarget = true;
                    break;
                }
            }

            if (!stillHasSameTarget)
            {
    ...
    public void CompleteDelivery(BoxCore box)
    {
        ...
        {
            rec.itemInstance.ownerNPC.HandleBoxStored();
        }
    ...
    public TimeSaveData CaptureTime()
    {
    ...
    public void RestoreTime(TimeSaveData data)
    {
        currentDay = data.day;
